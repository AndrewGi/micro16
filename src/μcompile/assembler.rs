...
impl<RawOut: std::io::Write> Assembler<RawOut> {
	pub fn new(byte_out: RawOut) -> Assembler<RawOut> {
		Assembler {
			out: byte_out,
			..Default()
		}
	...
	fn find_label(&self, label: &str) -> Option<u16> {
		Some(*self.labels.iter().find(|li: &(String, u16)|{
			return (li.0) == label;
		})?.1)
	}
	...
	fn process_line(&mut self, line: AssemblerLine) -> Option<()> {
		match line {
	...
	pub fn process_next(&mut self) -> Option<()> {
		self.process_line(self.next_line()?)
	}
}